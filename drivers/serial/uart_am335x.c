...
static int uart_am335x_configure(const struct device *dev, const struct uart_config *cfg)
{
...
static int uart_am335x_config_get(const struct device *dev, struct uart_config *cfg)
{
	const struct uart_am335x_config *config = dev->config;
...
static int uart_am335x_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	int i;
...
static int uart_am335x_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	int i;
...
static void uart_am335x_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
					 void *user_data)
{
...
#define UART_AM335X_IRQ_CONFIG(n)                                                                  \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), uart_am335x_isr,            \
...
#define UART_AM335X_INIT(n)                                                                        \
	...
	static struct uart_am335x_data uart_am335x_data_##n;                                       \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, uart_am335x_init, NULL, &uart_am335x_data_##n,                    \
			      &uart_am335x_config_##n, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,  \
			      &uart_am335x_api);
